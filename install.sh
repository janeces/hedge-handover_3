...
#sshpass -p $HOST_PWD ssh $HOST_UNAME@$HOST_NAME "mkdir -p $BASE_PATH"
ssh hedge_device "mkdir -p $BASE_PATH"  # Make the project root directory and create any missing parent directories.
#sshpass -p $HOST_PWD rsync -avz --exclude='__pycache__' --exclude='*.egg-info' $files "$HOST_UNAME@$HOST_NAME:$BASE_PATH"
...
# TODO: replace this with the commented line. It just governs how the connection is established.
#sshpass -p $HOST_PWD ssh $HOST_UNAME@$HOST_NAME "cd $BASE_PATH && ./scripts/local_install.sh $prefix"
